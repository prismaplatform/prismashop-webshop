...
import { Listbox, Transition } from "@/app/ui/headlessui";
import { CheckIcon, ChevronDownIcon } from "@heroicons/react/24/solid";

const DEMO_DATA = [
  { name: "Ordinea de sortare" },
  { name: "Cel mai nou rating" },
  { name: "Cel mai mare rating" },
  { name: "Cel mai mic rating" },
];

interface Props {
  data?: { name: string }[];
  className?: string;
  onChange?: (value: string) => void; // Added onChange prop to notify parent of selected value
}
...
const SortOrderFilter: FC<Props> = ({ data = DEMO_DATA, className = "", onChange }) => {
  ...
  const handleSelectionChange = (selectedOption: { name: string }) => {
    setSelected(selectedOption);
    if (onChange) {
      onChange(selectedOption.name); // Notify the parent component with the selected value
    }
  };
