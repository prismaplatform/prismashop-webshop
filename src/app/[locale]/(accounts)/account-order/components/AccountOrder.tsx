  ...
  useEffect(() => {
    if (currentUser?.id) {
      const fetchOrders = async () => {
        try {
          const ordersData = await getAllOrders(currentUser.id as number);
      ...
      >
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-4 sm:p-8 bg-menu-bg-dark">
          <div>
            ...
            <p className="text-menu-text-light dark:text-accent-400 text-sm mt-1.5 sm:mt-2">
              <span>{orderDate}</span>
          ...
          <div className="mt-3 sm:mt-0 flex flex-col space-y-2">
            {/* Return Button */}
            <ButtonSecondary
              sizeClass="py-2 px-4 sm:px-6 text-xs"
        ...
        <div className="border-t border-accent-200 dark:border-accent-700 p-2 sm:p-8 divide-y divide-y-accent-200 dark:divide-accent-700">
          {order.orderItems?.map((item: any, idx: number) => (
            <ProductItem key={idx} rootUrl={rootUrl} item={item} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10 sm:space-y-12">
      ...
      {orders.length > 0 ? (
        ...
        <p className="text-menu-text-light">{t("noOrders")}</p>
      )}
      <ReturnModal
        order={selectedOrder}
        isOpen={isOpen}
