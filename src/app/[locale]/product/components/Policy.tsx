...
// Assuming useTranslations is correctly set up for internationalization
import { useTranslations } from "use-intl";

// --- Policy Component ---
const Policy = () => {
  // Initialize translation hook
  const t = useTranslations("Pages.Products.DetailCards");

  // --- Feature Data ---
  // Updated structure slightly for clarity if needed, but keeping original data structure.
  // Added iconBgColor to separate card background from icon background for cleaner design.
  const A_FEATURES = [
    {
      color: "bg-primary-50",
      iconBgColor: "bg-primary-100",
      iconTextColor: "text-primary-500",
      name: t.raw("Card1.title"),
      desc: t.raw("Card1.text"),
      // SVG string remains the same
    ...
    },
    {
      ...
      iconBgColor: "bg-primary-100",
      iconTextColor: "text-primary-500",
      name: t.raw("Card2.title"),
      desc: t.raw("Card2.text"),
      svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6">
    ...
    },
    {
      ...
      iconBgColor: "bg-primary-100",
      iconTextColor: "text-primary-500",
      name: t.raw("Card3.title"),
      desc: t.raw("Card3.text"),
      svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6">
    ...
    },
    {
      ...
      iconBgColor: "bg-primary-100",
      iconTextColor: "text-primary-500",
      name: t.raw("Card4.title"),
      desc: t.raw("Card4.text"),
      svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative font-['Inter',_sans-serif]">
      {A_FEATURES.map((item, index) => {
        return (
          <div
            key={index}
            className={`flex flex-col p-6 bg-menu-bg-light rounded-2xl border border-menu-text-light transition-shadow hover:shadow-lg`}
          >
            <div className={`inline-flex rounded-lg ${item.iconTextColor}`}>
              <div dangerouslySetInnerHTML={{ __html: item.svg }} />
            </div>

            <div className="mt-2">
              <div
                className="font-semibold text-menu-text-light"
                dangerouslySetInnerHTML={{ __html: item.name }}
              />
              <div
                className="text-menu-text-light mt-1 text-sm"
